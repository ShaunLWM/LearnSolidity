...
contract AuctionHouse is Ownable, ReentrancyGuard {
  ...
  function newAuction(
    ...
    require(
      ((timeEnd - timeStart) >= MIN_LENGTH_AUCTION) && ((timeEnd - timeStart) < MAX_LENGTH_AUCTION),
      "Aunction length outside of range"
    );
    require(amount >= minAuctionStartBid, "Auction must be more than minAuctionStartBid");
  ...
  function addBid(uint256 auctionId) external payable {
    ...
    require(currentAuction.createdBy != msg.sender, "Creator cannot bid own auction");

    require(block.timestamp >= currentAuction.timeStart, "Auction has not started");

    require(block.timestamp < currentAuction.timeEnd, "Auction has already ended");

    require(
      msg.value > currentAuction.basePrice && msg.value > currentAuction.currentHighestBid.price,
      "Your bid price must be higher than current highest bid price"
    );

    if (currentAuction.bidCount > 0) {
      pendingWithdraws[currentAuction.currentHighestBid.user] += currentAuction.currentHighestBid.price;
    }

    Bid memory newBit = Bid({bidTime: block.timestamp, price: msg.value, user: msg.sender});
